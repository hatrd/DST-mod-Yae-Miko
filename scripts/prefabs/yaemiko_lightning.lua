-- 由饥荒lightning.lua复制，用于修改音效、视频特效。
local assets =
{
    Asset("ANIM", "anim/lightning.zip"),
}

local LIGHTNING_MAX_DIST_SQ = 70*70
...
local function PlayThunderSound(lighting)
    ...
    inst.SoundEmitter:PlaySound("yaemiko_sfx/yaemiko_sfx/yaemiko_ehit"..math.random(3), nil, intensity, true)
...
local function StartFX(inst)
	for i, v in ipairs(AllPlayers) do
		...
		local k = math.max(0, math.min(1, distSq / LIGHTNING_MAX_DIST_SQ))
		local intensity = -(k-1)*(k-1)*(k-1)/2				--k * 0.8 * (k - 2) + 0.8

		--print("StartFX", k, intensity)
		if intensity > 0 then
			v:ScreenFlash(intensity <= 0.05/2 and 0.05/2 or intensity)
...
return Prefab("yaemiko_lightning", fn, assets)